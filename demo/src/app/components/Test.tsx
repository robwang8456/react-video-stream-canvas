...
export function Toggler({ title, children }: TogglerProp) {
  ...
  return (
    <>
      ...
      {show && <div className="m-0 mt-2">{children}</div>}
    </>
  );
}
...
export function DemoItem({
  ...
  return (
    <div className="ms-4 m-2 pe-0 p-2 w-128 border-2">
      <Toggler title={title}>{test}</Toggler>
    </div>
  );
}
  ...
  return (
    <>
      <div className="text-black font-bold py-1 px-4 rounded text-3xl">
        ReactVSC: react-video-stream-canvas
      </div>
      <div className="ms-4 text-blue-500 font-bold py-1 px-4 rounded text-md"></div>
      <div className="ms-4 text-red-500 font-bold py-1 px-4 rounded text-md">
        {cameraCount == 0 && "**Camera is need for most of demos."}
        {cameraCount > 1 &&
          "**There are 2|2+ cameras, we will use the default one."}
      </div>
